...
func printArea(_ a : Double)->Double
{
    return 2*a*3.14
}
...
func checkLoginStatus(_ status: Bool, _ name: String?) {
    if status {
...
func CheckLoginStatus(_ status: Bool, _ name: String?) {
    guard status else {
        print("로그인이 필요합니다.") // 로그인 상태가 false일 때 출력
        return
    }
    
    guard let userName = name else {
        print("알 수 없는 사용자") // 사용자 이름이 nil일 때 출력
        return
    }
    
    print(userName) // 사용자 이름이 nil이 아닐 때 출력
}

CheckLoginStatus(true, "HSH")
CheckLoginStatus(true, nil)
CheckLoginStatus(false, "HSH")
